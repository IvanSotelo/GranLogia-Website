...
describe('Contact.vue', () => {
  ...
  it('should render correct contents', () => {
    ...
    expect(wrapper.findAll('.col-6').length).toBe(14)
    const firstColumn = columns.at(2)
    expect(firstColumn.find('p').text()).toBe('Archive materials')
    const secondColumn = columns.at(4)
    expect(secondColumn.find('p').text()).toBe('Music')
    const thirdColumn = columns.at(6)
    expect(thirdColumn.find('p').text()).toBe('Development')
    const fourColumn = columns.at(8)
